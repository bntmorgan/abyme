...
void elf64_load_segment(Elf64_Ehdr *elf64_header, uint8_t *destination) {
  ...
  source = (uint8_t *) (((uintptr_t) elf64_header) + ((uintptr_t)program_header->p_offset));
  /*
  ...
  if (program_header->p_memsz > program_header->p_filesz) {
    memset(destination + program_header->p_filesz, 0, program_header->p_memsz - program_header->p_filesz);
  }
